...
function App() {
  ...
  const readEnabled = isConnected && isContractConfigured && Boolean(address)
  ...
  const {
    ...
    chainId: base.id,
    query: { enabled: readEnabled, refetchInterval: 12_000 },
  })

  const { data: globalEntries, refetch: refetchEntries } = useReadContract({
    address: STILLPOINT_ADDRESS,
    abi: stillpointAbi,
  ...
  const checkedIn = Number(profile?.lastCheckInDay || 0n) === todayUtc()
  ...
  const statusText = useMemo(() => {
    ...
    if (isConfirmed && pendingAction === 'state') return 'State recorded'
    if (isConfirmed && pendingAction === 'checkin') return 'Checked in for today'
    return ''
  }, [isSwitching, isSending, isConfirming, isConfirmed, pendingAction])

  useEffect(() => {
    if (!isConfirmed) return
    void Promise.all([refetchProfile(), refetchEntries(), refetchCheckIns()])
  }, [isConfirmed, refetchProfile, refetchEntries, refetchCheckIns])
  ...
  async function runAction(action: Exclude<PendingAction, null>) {
    ...
    try {
      ...
      const data = encodeFunctionData({
        abi: stillpointAbi,
        functionName: action === 'state' ? 'recordState' : 'dailyCheckIn',
    ...
    <div className="app-shell">
      ...
      <main id="top">
        <section className="intro">
          ...
          <h1>How are you,<br />right now?</h1>
          <p className="intro-copy">
            Choose the state that feels closest. Leave one quiet signal onchain.
          </p>
        </section>

        <section className="state-studio" aria-label="Choose your current state">
          ...
          <div className="state-selector">
            {states.map((state, index) => (
              <button
                key={state.name}
                className={index === selected ? 'state-option active' : 'state-option'}
          ...
          >
            <span>{entriesToday >= 5 ? 'Five moments recorded' : `Record ${selectedState.name.toLowerCase()}`}</span>
            {busy && pendingAction === 'state' ? <span className="spinner" /> : <ArrowRight size={20} />}
          </button>
          ...
          <div className="daily-cap">
            ...
            <div className="cap-dots" aria-hidden="true">
              {[0, 1, 2, 3, 4].map((item) => (
                <span key={item} className={item < entriesToday ? 'filled' : ''} />
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="checkin-section">
          <div className="section-heading">
            ...
            <span className="utc-label">Resets 00:00 UTC</span>
          </div>

          <div className="checkin-row">
            <div className={checkedIn ? 'checkin-symbol complete' : 'checkin-symbol'}>
              {checkedIn ? <Check size={27} /> : <CircleDot size={27} />}
            </div>
            <div className="checkin-copy">
              <strong>{checkedIn ? 'Today is complete' : 'Daily check-in'}</strong>
              <span>{checkedIn ? 'Come back tomorrow to continue.' : 'Keep a simple, unbroken rhythm.'}</span>
            </div>
            <button
            ...
            >
              {busy && pendingAction === 'checkin' ? <span className="spinner dark" /> : checkedIn ? <Check size={20} /> : <ChevronRight size={22} />}
            </button>
          </div>
          ...
          <div className="metrics">
            ...
            <div className="metric">
              ...
              <strong>{Number(profile?.totalEntries || 0n)}</strong>
            </div>
          </div>
        </section>
        ...
        <section className="community-strip">
          <div>
            ...
            <strong>{Number(globalEntries || 0n).toLocaleString()}</strong>
          </div>
          <div>
            <span>Daily returns</span>
            <strong>{Number(globalCheckIns || 0n).toLocaleString()}</strong>
          </div>
          <p>No app fee. No token. Only Base network gas.</p>
        </section>
      ...
      {walletOpen && (
        <div className="modal-backdrop" onMouseDown={() => setWalletOpen(false)}>
          <div className="wallet-modal" role="dialog" aria-modal="true" aria-labelledby="wallet-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <span className="modal-symbol"><Wallet size={23} strokeWidth={1.6} /></span>
            <p className="eyebrow">Continue on Base</p>
            <h2 id="wallet-title">Choose a wallet</h2>
            <p className="modal-copy">Connect to record your daily state and check-in.</p>
            <div className="wallet-options">
              <button onClick={() => connectWallet(1)} disabled={isConnecting || isReconnecting}>
                <span className="wallet-logo base-logo">B</span>
                <span><strong>Base Account</strong><small>Coinbase smart wallet</small></span>
                <ChevronRight size={21} />
              </button>
              <button onClick={() => connectWallet(0)} disabled={isConnecting || isReconnecting}>
                <span className="wallet-logo browser-logo"><Wallet size={20} /></span>
                <span><strong>Browser wallet</strong><small>MetaMask, Rabby and more</small></span>
                <ChevronRight size={21} />
              </button>
            </div>
          </div>
